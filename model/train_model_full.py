...
# ========== ProtoPNet Feature Engineering (from protopnetrain.py) ==========

def get_feature_names():
    """Get feature names in order matching the CSV columns"""
    base_features = ['catches', 'dots', 'economy', 'fours', 'runouts', 'runs',
                     'sixes', 'strike_rate', 'stumpings', 'wickets', 'fantasy_points']
    time_periods = ['A3', 'A5', 'A8', 'AC']
    feature_names = []
    for period in time_periods:
        for feat in base_features:
            feature_names.append(f"{period}_{feat}")
...
def get_domain_indices():
    ...
    for i in range(4):  # 4 time periods
        offset = i * 11
        batting += [offset + j for j in [3, 5, 6, 7, 10]]  # fours, runs, sixes, strike_rate, fantasy
        bowling += [offset + j for j in [1, 2, 9, 10]]     # dots, economy, wickets, fantasy
        fielding += [offset + j for j in [0, 4, 8, 10]]    # catches, runouts, stumpings, fantasy
...
if TORCH_AVAILABLE:
    ...
    class CricketDataset(torch.utils.data.Dataset):
        def __init__(self, df, feature_cols, target_cols, feat_norm, targ_norm):
            self.feat_norm = feat_norm
            self.targ_norm = targ_norm
            self.X_raw = df[feature_cols].values.astype(np.float32)
            self.y_raw = df[target_cols].values.astype(np.float32)
        
        def __len__(self):
            return self.X_raw.shape[0]
        
        def __getitem__(self, idx):
            in_feats = self.feat_norm(self.X_raw[idx])
            combos = generate_feature_combos(in_feats)
            targets = self.targ_norm(self.y_raw[idx])
            return torch.tensor(combos, dtype=torch.float32), torch.tensor(targets, dtype=torch.float32)
...
class Dream11ModelTrainer:
    ...
    def __init__(self, data_path, model_artifacts_dir='model_artifacts', mode='traditional'):
        """
        Args:
            data_path: Path to training CSV
            model_artifacts_dir: Directory to save models
            mode: 'traditional' (XGB/LGBM/CB) or 'protopnet' (PyTorch ProtoPNet)
        """
    ...
    def train_protopnet(self, device='cuda', num_epochs=50, delta=1.0):
        """Train ProtoPNet model (if PyTorch available)"""
        if not TORCH_AVAILABLE:
            print("⚠️  PyTorch not available, skipping ProtoPNet")
            return None
        
        print("\n" + "=" * 100)
        print("[ProtoPNet] TRAINING")
        print("=" * 100)
        ...
        # Implementation here...
        print("✅ ProtoPNet training complete (stub - full implementation from protopnetrain.py)")
    ...
    def plot_feature_importance(self):
        """Plot feature importances from ensemble models"""
        importances = {}
    ...
    def plot_performance_by_match_type(self, ensemble_pred):
        """Compare performance across T20 and ODI"""
    ...
    def plot_performance_by_role(self, ensemble_pred):
        """Performance breakdown by player role"""
        test_df = self.test_df.copy()
        test_df['prediction'] = ensemble_pred
        test_df['error'] = abs(test_df['fantasy_points'] - test_df['prediction'])
    ...
    def plot_learning_curves(self):
        """Plot learning curves for ensemble models"""
    ...
    def plot_performance_over_time(self, ensemble_pred):
        """Performance trend over test period"""
        test_df = self.test_df.copy()
        test_df['prediction'] = ensemble_pred
        test_df['error'] = abs(test_df['fantasy_points'] - test_df['prediction'])
    ...
    def plot_prediction_confidence(self, ensemble_pred):
        ...
        if len(preds) < 2:
            print("⚠️  Not enough models for confidence intervals")
            return
    ...
    def train_full_pipeline(self, model_name='ProductUIModel'):
        ...
        print("\n" + "=" * 100)
        print(f"DREAM11 TRAINING PIPELINE - MODE: {self.mode.upper()}")
        print("=" * 100)
        ...
        if self.mode == 'traditional':
            ...
            self.train_protopnet()
            # ProtoPNet-specific evaluation and plotting
        ...
        if self.mode == 'traditional':
            print(f"Best Model: Ensemble (Val MAE: {self.ensemble_mae:.2f})")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Dream11 Model Training')
    parser.add_argument('--data', default='data/processed/training_data_all.csv', help='Training data CSV path')
    parser.add_argument('--mode', choices=['traditional', 'protopnet'], default='traditional', 
                       help='Training mode: traditional (XGB/LGBM/CB) or protopnet (PyTorch)')
    parser.add_argument('--name', default='ProductUIModel', help='Model artifact name prefix')
    ...
    trainer.train_full_pipeline(model_name=args.name)
