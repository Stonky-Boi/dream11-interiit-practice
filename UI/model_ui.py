...
For judging model performance on specified date ranges
"""
...
class ModelUI:
    ...
    def train_model(self, train_start, train_end, model_name):
        ...
        st.write(f"Date range: {train_df['date'].min()} to {train_df['date'].max()}")
    ...
    def evaluate_model(self, model_name, test_start, test_end):
        ...
        try:
            ...
            test_data_path = self.processed_dir / 'test_data_after_2024-06-30.csv'
            
            if test_data_path.exists():
                st.success("✓ Using processed test data")
                df = pd.read_csv(test_data_path)
                df['date'] = pd.to_datetime(df['date'])
            else:
                # Fall back to training data
                st.warning("⚠️ Test data not found. Using training data.")
                st.info("Run feature engineering on test data: python data_processing/feature_engineering.py")
                data_path = self.processed_dir / 'training_data_2024-06-30.csv'
                if not data_path.exists():
    ...
    def run(self):
        ...
        st.markdown("### Evaluate Model Performance on Custom Date Ranges")
        st.markdown("---")
        ...
        **Important Guidelines:**
        ...
        # Validation - convert train_end to datetime.date for comparison
        ...
        if train_button:
            ...
            if trainer:
                ...
                # Model performances
                st.markdown("### Individual Model Performances")
                perf_data = []
                for name, model_dict in trainer.models.items():
                    perf_data.append({
                        'Model': name.capitalize(),
                        'MAE': f"{model_dict['val_mae']:.2f}",
                        'RMSE': f"{model_dict['val_rmse']:.2f}",
                        'R²': f"{model_dict['val_r2']:.4f}"
                    })
        ...
        if eval_button:
            ...
            if trainer:
                ...
                # Evaluate
                results_df = self.evaluate_model(
                    model_name,
                    test_start,
                    test_end
                )
